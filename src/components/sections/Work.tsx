        ...
        {projects.map((project) => (
          ...
          >
            ...
            <div className="grid min-h-[250px] grid-cols-[1.1fr_.75fr_.55fr] items-center gap-[35px] max-[900px]:grid-cols-[1fr_.65fr] max-sm:block max-sm:min-h-0 max-sm:py-8 max-sm:pb-6">
              ...
              <div
                className="relative h-[170px] scale-[.92] -rotate-2 overflow-hidden bg-[var(--accent)] transition-transform duration-500 group-hover:scale-100 group-hover:rotate-1 max-[900px]:row-span-2 max-[900px]:[grid-area:1/2/3] max-sm:mb-[22px] max-sm:h-[210px] max-sm:scale-100 max-sm:-rotate-1"
                aria-hidden="true"
              >
            ...
            <div className={`flex gap-2 max-sm:flex-wrap ${mono}`}>
              {project.tags.map((tag) => (
                <span
                  className="rounded-full border border-ink/25 px-[9px] py-1.5"
                  key={tag}
                >
                  {tag}
                </span>
